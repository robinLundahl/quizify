                      ...
                      <div className="flex flex-1 flex-col p-5">
                        ...
                        <div className="mt-auto pt-4">
                          <div className="flex flex-wrap gap-2 border-t border-gray-100 dark:border-gray-700 pt-4">
                            <button onClick={() => navigate(`/quiz/${quiz.id}`)} className="rounded-xl bg-indigo-50 dark:bg-indigo-900/30 px-3 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors">
                              {t('common.edit')}
                            </button>
                            <button onClick={() => handleHost(quiz.id)} disabled={hostingId === quiz.id} className="flex-1 rounded-xl bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50 transition-colors">
                              {hostingId === quiz.id ? t('common.creating') : t('dashboard.host')}
                            </button>
                            {isPublished ? (
                              <button onClick={() => unpublishListing.mutate(listing!.id)} disabled={unpublishListing.isPending} className="rounded-xl border border-gray-200 dark:border-gray-700 px-3 py-2 text-sm text-gray-500 dark:text-gray-400 transition hover:border-amber-200 hover:bg-amber-50 hover:text-amber-600 disabled:opacity-50">
                                {t('dashboard.unpublish')}
                              </button>
                            ) : (
                              <button onClick={() => setPublishingQuiz({ id: quiz.id, title: quiz.title, category: quiz.category, language: quiz.language })} className="rounded-xl border border-indigo-200 dark:border-indigo-700 px-3 py-2 text-sm font-medium text-indigo-600 dark:text-indigo-400 transition hover:bg-indigo-50 dark:hover:bg-indigo-900/20">
                                {t('dashboard.publish')}
                              </button>
                            )}
                            <button onClick={() => setConfirmDelete({ id: quiz.id, title: quiz.title })} className="rounded-xl border border-gray-200 dark:border-gray-700 px-3 py-2 text-sm text-gray-500 dark:text-gray-400 transition hover:border-red-200 hover:bg-red-50 hover:text-red-600">
                              {t('common.delete')}
                            </button>
                          </div>
                        </div>
                          ...
                          <div className="mt-4 border-t border-gray-100 dark:border-gray-700 pt-3 space-y-1.5">
                            ...
                            {quiz.sessions.map((s) => (
                              <div key={s.id} className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  <Link to={`/results/${s.id}`} className="text-xs font-medium text-indigo-600 hover:text-indigo-700">{t('dashboard.results')}</Link>
